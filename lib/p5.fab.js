...
// p5.prototype.autoHeat = function() {
//   _fab.serial.on('open', () => _fab.serial.write("M104 S205\n") );
// };
 ...
 */
class Fab {
  constructor({baudRate = 115200, nozzleRadius = 0.4 / 2, filamentRadius = 1.75 / 2, maxX = 220, maxY = 220, maxZ = 250, autoConnect=true, extrusionMultiplier = 1}={}) {
  ...
  */
  ...
  moveExtrude(x, y, z, v = 25, e = this.extrusionMultiplier * this.makeE(x, y, z)) {
    this.x = parseFloat(x).toFixed(2);
    this.y = parseFloat(y).toFixed(2);
    this.z = parseFloat(z).toFixed(2);
    v = this.mm_sec_to_mm_min(v);
  ...
  autoHeat(tNozzle, tBed) {
    _fab.serial.write(`M104 S${tNozzle}\n`); // set nozzle temp without waiting
    _fab.serial.write(`M140 S${tBed}\n`); // set bed temp without waiting
  }
}
